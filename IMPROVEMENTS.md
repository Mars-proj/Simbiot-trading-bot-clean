...
- **`core.py`**:
  1. Вынести API-ключи в `.env` файл для повышения безопасности.
  2. Создать модуль `user_manager.py` для управления пользователями через Redis или PostgreSQL, чтобы масштабировать систему до 1000+ пользователей.
  3. Добавить обработку ошибок для `filter_symbols` и `start_trading_all`.
- **`worker.py`**:
...
- **`start_trading_all.py`**:
  1. Добавить динамическую сумму ордера на основе минимального объёма транзакции (через `symbol_data_fetcher.py`).
  2. Добавить проверку баланса перед размещением ордера.
- **`bot_trading.py`**:
...
### Работа с биржей
- **`exchange_pool.py`**:
  1. Создать пул подключений к MEXC (ограничить до 10 одновременных подключений).
...
### Фильтрация символов
- **`symbol_filter.py`**:
  1. Заменить фиксированный порог фильтрации (`len(result) >= 89`) на динамический.
  2. Кэшировать результаты фильтрации в Redis.
...
- **`config_keys.py`**:
  1. Перенести все ключи в `.env` файл.
  2. Позволить настраивать ключи через API.
- **`bot_user_data.py`**:
  1. Перенести данные пользователей в Redis или PostgreSQL.
