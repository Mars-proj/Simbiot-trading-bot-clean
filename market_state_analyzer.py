...
async def analyze_market_state(exchange, timeframe='1h'):
    ...
    for attempt in range(retry_count):
        try:
            ...
            logger.error(f"Timeout while fetching ticker on attempt {attempt + 1}/{retry_count}")
            if attempt < retry_count - 1:
                logger.info(f"Retrying in 5 seconds...")
                await asyncio.sleep(5)
            else:
                logger.error("Failed to fetch ticker after all retries")
                return {'trend': 'neutral', 'volatility': 0.01}
        except Exception as e:
            logger.error(f"Failed to analyze market state on attempt {attempt + 1}/{retry_count}: {e}")
            if attempt < retry_count - 1:
                logger.info(f"Retrying in 5 seconds...")
                await asyncio.sleep(5)
            else:
