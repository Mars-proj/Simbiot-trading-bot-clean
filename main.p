...
async def run_trading_for_user(user, exchange_id, model_path, backtest_days, min_profit_threshold, exchange_pool, symbols, backtest_results):
    ...
    try:
        ...
        async def data_loader():
            ...
            # Используем BTCUSDT вместо KPNUSDT для переобучения
            retrain_symbol = "BTCUSDT"
            logger_main.info(f"Fetching historical data for retraining using symbol {retrain_symbol}")
            data = await fetch_historical_data(exchange_id, user_id, retrain_symbol, since=int(time.time()) - 30*24*60*60, testnet=testnet, exchange=exchange)
            if data is None:
